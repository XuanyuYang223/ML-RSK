#!/usr/bin/env python3
"""Train a learned reranker over partition-classifier top-k candidates."""
...
class PartitionPairReranker(nn.Module):
    def __init__(
        self,
        n: int,
        d_model: int,
        ...
        self.perm_pos_embedding = nn.Embedding(n + 1, d_model)
        ...
        self.perm_encoder = nn.TransformerEncoder(encoder_layer, num_layers=num_layers)
        ...
        self.pair_mlp = nn.Sequential(
            nn.LayerNorm((4 * d_model) + 1),
            nn.Linear((4 * d_model) + 1, dim_feedforward),
            nn.GELU(),
            nn.Dropout(dropout),
    ...
    def encode_permutation(self, permutations: torch.Tensor) -> torch.Tensor:
        ...
        tokens = torch.cat([cls, self.perm_embedding(permutations)], dim=1)
        positions = torch.arange(n + 1, device=permutations.device)
        encoded = self.perm_encoder(tokens + self.perm_pos_embedding(positions)[None, :, :])
        return encoded[:, 0]
    ...
    def forward(
        ...
        expanded_perm = perm_repr[:, None, :].expand_as(part_repr)
        features = torch.cat(
            [
                expanded_perm,
                part_repr,
                expanded_perm * part_repr,
                (expanded_perm - part_repr).abs(),
                classifier_log_probs[..., None],
            ],
            dim=-1,
        )
        learned_scores = self.pair_mlp(features).squeeze(-1)
...
def run_epoch(
    reranker: PartitionPairReranker,
    classifier: PartitionClassifier,
    loader: DataLoader,
    ...
    for permutations, labels, true_shapes in loader:
        ...
        with torch.set_grad_enabled(is_train):
            ...
            if has_hit.any():
                hit_scores = scores[has_hit]
                hit_targets = hit_mask[has_hit].float().argmax(dim=1)
                loss = nn.functional.cross_entropy(hit_scores, hit_targets)
            else:
                loss = scores.sum() * 0.0
    ...
    loss_denominator = max(total_hit_examples, 1)
    ...
    return total_loss / loss_denominator, metrics
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--model-out", type=Path, default=Path("experiments/rsk_shape_learned_reranker/model.pt"))
    parser.add_argument("--results-out", type=Path, default=Path("experiments/rsk_shape_learned_reranker/results.json"))
...
def main() -> None:
    ...
    print(
        ...
        f"device={device} parameters={sum(p.numel() for p in reranker.parameters())}",
        flush=True,
    )
